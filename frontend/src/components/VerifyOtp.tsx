...
const VerifyOtp = () => {
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLElement>):void => {
    ...
    const digits = pastedData.replace(/\D/g, "").slice(0, 6); // Remove non-digit characters
    if(digits.length === 6) {
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-8">
          <div className="text-center mb-8 relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <div className="flex justify-center in-checked: space-x-3">
                  {otp.map((digit, index) => (
                    <input
                      key={index}
                      ref = {(e1: HTMLInputElement | null) =>{
                        (inputRefs.current[index] = e1)
                      } }
                      type="text"
                      maxLength={1}
                      value={digit}
                      ...
                      className="w-12 h-12 text-center text-xl font-bold bg-gray-700 border-2 border-gray-600 rounded-lg text-white "
                    />
                  ))}
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 text-white py-4 px-6
